...
interface ShowsInfiniteScrollProps {
  initialShows: Show[];
}

export function ShowsInfiniteScroll({ initialShows }: ShowsInfiniteScrollProps) {
  ...
  const fetchMoreShows = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching more shows", e);
      setHasMore(false);
    } finally {
      setLoading(false);
    }
        ...
        >
          {loading && (
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground animate-pulse">Syncing Series Database...</p>
            </div>
          )}
        </div>
      )}
